...
if (strpos($_SERVER['HTTP_HOST'], "ipd24.ca") !== false) {
    ...
    DB::$dbName = 'cp5003_project';
    DB::$user = 'cp5003_project';
    DB::$password = 'ORgs4EbGABwa';
} else { // local computer
...
$app->get('/', function ($request, $response, $args) {
    $articleList = DB::query("SELECT klfjsadl a.id, a.authorId, a.creationTS, a.title, a.body, u.name "
...
$app->get('/article/{id:[0-9]+}', function (Request $request, Response $response, $args) {
    $id = $args['id'];
    $loginLink = '/login?returnUrl=/article/' + $id;
    // TODO: render template, tell user to login to be able to post comments
}); */
...
$app->post('/login', function (Request $request, Response $response, $args) use ($log) {
    ...
    if (!$loginSuccess) {
        ...
        if ($returnUrl) {
            ...
            ['userSession' => $_SESSION['blogUser'], 'returnUrl' => $returnUrl ] );
        }
    }
});
...
$app->add(function (ServerRequestInterface $request, ResponseInterface $response, callable $next) {
    ...
    if (startsWith($url, "/admin")) {
        if (!isset($_SESSION['blogUser']) || $_SESSION['blogUser']['isAdmin'] != 1) { // refuse if user not logged in AS ADMIN
            $response = $response->withStatus(403);
